...
var DEVICE_CODE_ENDPOINT = "https://oauth2.googleapis.com/device/code";
var REVOKE_ENDPOINT = "https://oauth2.googleapis.com/revoke";
function createPkcePair(random = randomBytes2) {
...
function createState(random = randomBytes2) {
...
function buildAuthorizationUrl(args) {
  ...
  if (args.loginHint !== void 0) url.searchParams.set("login_hint", args.loginHint);
  return url.toString();
}
...
function readRedirect(url, expectedState) {
...
async function loginWithLoopback(deps, options = {}) {
  const clock = deps.clock ?? systemClock;
  const notify = options.onMessage ?? (() => void 0);
...
function waitForRedirect(server, state, timeoutMs, audience) {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      reject(
        new FatalError(
          "timed out waiting for the Google sign-in redirect",
          audience === void 0 ? REAUTH_REMEDIATION : `Nothing came back from Google. The usual cause is signing in with an account this build does not accept: it only allows ${audience}. Google blocks other accounts on its own page and never returns here. To use a different account, supply your own OAuth client \u2014 see the README.`
        )
      );
    }, timeoutMs);
    timer.unref();
...
function refusalHelp(error, audience) {
  const refused = error === "access_denied" || error === "org_internal";
  if (refused && audience !== void 0) {
    return `That account was refused. This build only accepts ${audience}. To archive from another account, supply your own OAuth client \u2014 see the README.`;
  }
  return REAUTH_REMEDIATION;
}
function resultPage(ok, error) {
  const title = ok ? "Signed in" : "Sign-in failed";
  const body = ok ? "You can close this tab and go back to your terminal." : `Something went wrong: ${error ?? "unknown error"}. Go back to your terminal and try again.`;
  return `<!doctype html><meta charset="utf-8"><title>${title}</title>
<body style="font-family:system-ui;margin:4rem auto;max-width:30rem">
<h1>${title}</h1><p>${body}</p></body>`;
}
async function exchangeCode(deps, args, clock, signal) {
  const response = await deps.http.send(TOKEN_ENDPOINT, {
    method: "POST",
    headers: { "content-type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({
  ...
  const tokens = { ...parsed, clientId: deps.client.clientId };
  await deps.tokenStore.write(tokens);
  return tokens;
}
async function loginWithDeviceCode(deps, options) {
  const clock = deps.clock ?? systemClock;
  ...
  const verificationUrl = asString(start?.["verification_url"]) ?? asString(start?.["verification_uri"]);
  ...
  let intervalMs = (typeof start?.["interval"] === "number" ? start["interval"] : 5) * 1e3;
  options.onPrompt({ userCode, verificationUrl, expiresInSeconds: expiresIn });
  const deadline = clock.now() + expiresIn * 1e3;
...
function describeAuthError(body, status) {
...
function listRetainedBundles(db, sessionId) {
  const rows = sessionId === void 0 ? db.prepare("SELECT * FROM retained_bundles ORDER BY id").all() : db.prepare("SELECT * FROM retained_bundles WHERE session_id = ? ORDER BY id").all(sessionId);
  return rows.map((row) => ({
    id: row.id,
...
function getFiles(db, sessionId, limit = 50) {
...
function claim(db, now, visibilityMs) {
  const token = randomBytes3(8).toString("hex");
  const row = db.prepare(
    `UPDATE jobs
          SET visible_at  = ?,
              attempts    = attempts + 1,
...
function createHashTee(algorithms = ["sha256", "md5"]) {
  ...
  for (const algorithm of algorithms) hashes.set(algorithm, createHash2(algorithm));
...
async function createBundle(input) {
  ...
  try {
    ...
    const sink = fs7.createWriteStream(temp, { flags: "wx", mode: 384 });
...
async function bundleOrExplain(ctx, session, work) {
...
async function buildBundle(ctx, session, index, now) {
  ...
  const outputPath = path12.join(ctx.paths.stagingDir, `${session.sessionId}.building.tar.zst`);
  const result = await bundleOrExplain(
    ctx,
    session,
    () => createBundle({
...
async function publish(ctx, job, session, bundle, index, previous, now) {
  ...
  const contentProblem = await bundleOrExplain(
    ctx,
    session,
...
function isStale(dir, staleMs, clock, owner) {
  let mtimeMs;
  try {
    mtimeMs = fs8.statSync(dir).mtimeMs;
  } catch {
    return true;
  }
...
// src/adapters/lazy-drive.ts
function createLazyDrive(factory) {
  let pending;
  const resolve = () => {
    pending ??= factory();
    return pending;
...
function commandContext(runtime, options = {}) {
  return {
    db: runtime.db(),
    paths: runtime.paths,
    ...
    signal: AbortSignal.timeout(options.timeoutMs ?? 10 * 6e4)
  };
}

// src/commands/output.ts
function ignoreClosedPipe() {
...
function formatDate(epochMs) {
  if (epochMs === null || epochMs === void 0 || epochMs <= 0) return "\u2014";
  return new Date(epochMs).toISOString().replace("T", " ").slice(0, 16);
}
function formatRelative(epochMs, now) {
  if (epochMs === null || epochMs === void 0 || epochMs <= 0) return "never";
  const seconds = Math.round((now - epochMs) / 1e3);
...
// src/commands/now.ts
async function runNow(runtime, options = {}) {
...
var WEIGHTS = { title: 6, summary: 4, prompt: 3, project: 2, branch: 2, file: 1 };
function scoreCandidate(session, prompts, terms) {
  if (terms.length === 0) {
...
function addMonths(epochMs, months) {
...
function runSearch(runtime, options) {
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.matchedPrompts[0] !== void 0) {
...
function toCard(runtime, candidate, includeFiles) {
  const session = candidate.session;
  return {
    sessionId: session.sessionId,
    title: session.title,
...
function resolveSessionId(runtime, query) {
  if (UUID.test(query)) return query;
  if (!/^[0-9a-f-]{6,}$/i.test(query)) return null;
  const row = runtime.db().prepare("SELECT session_id FROM sessions WHERE session_id LIKE ? LIMIT 2").all(`${query}%`);
...
// src/commands/setup.ts
async function runSetup(runtime, options = {}) {
  const steps = {};
...
async function runVerify(runtime, options = {}) {
...
// src/commands/cli.ts
var USAGE = `claude-archive <command> [options]
...
async function main() {
  ...
  if (values.help === true || command === void 0 || command === "help") {
    print(USAGE);
    return command === void 0 && values.help !== true ? 1 : 0;
  }
