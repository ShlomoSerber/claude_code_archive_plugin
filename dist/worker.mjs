...
function resolveDataDir(env, claudeDir) {
  ...
  return path7.join(claudeDir, "plugins", "data", DEFAULT_PLUGIN_SLUG);
}
...
async function readConfigFile(dataDir) {
  try {
    const raw = await fsp5.readFile(path8.join(dataDir, "config.json"), "utf8");
    const parsed = JSON.parse(raw);
...
async function uploadCatalogCopy(ctx) {
  const destination = path13.join(ctx.paths.stagingDir, "catalog.sqlite");
  try {
